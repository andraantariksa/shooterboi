...
impl Scene for GameSelectionScene {
    ...
    fn update(
...

        {
                ...
                .flow_down(&[
                    (
                        ...
                        conrod_core::widget::Canvas::new().flow_right(&[
                            (
                                self.ids.game_mode_selection_canvas,
                                conrod_core::widget::Canvas::new(),
                            ),
                            (
                                self.ids.game_detail_canvas,
                                conrod_core::widget::Canvas::new().flow_down(&[
                                    (
                                        self.ids.title_text_canvas,
                                        conrod_core::widget::Canvas::new().length(60.0),
                                    ),
                                    (
                                        self.ids.description_text_canvas,
                                        conrod_core::widget::Canvas::new().length(300.0),
                                    ),
                                    (
                                        self.ids.play_button_canvas,
                                        conrod_core::widget::Canvas::new()
            ...
            conrod_core::widget::Text::new(game_modes[self.selected_game_mode_idx].title)
                .middle_of(self.ids.title_text_canvas)
                .padded_w_of(self.ids.title_text_canvas, MARGIN)
                // .align_middle_x_of(self.ids.title_text_canvas)
                // .center_justify()
                .set(self.ids.title_text, &mut ui_cell);
            conrod_core::widget::Text::new(game_modes[self.selected_game_mode_idx].description)
                ...
                // .align_middle_x_of(self.ids.description_text_canvas)
                // .center_justify()
                .set(self.ids.description_text, &mut ui_cell);
            play_button = conrod_core::widget::Button::new()
                .label("Play")
                .middle_of(self.ids.play_button_canvas)
                .padded_w_of(self.ids.play_button_canvas, MARGIN)
                // .align_middle_x_of(self.ids.play_button_canvas)
                .wh(conrod_core::Dimensions::new(BUTTON_WIDTH, BUTTON_HEIGHT))
                .set(self.ids.play_button, &mut ui_cell);

            conrod_core::widget::Text::new("Score")
                .mid_top_of(self.ids.game_score_canvas)
                .padded_w_of(self.ids.game_score_canvas, MARGIN)
                .align_middle_x_of(self.ids.game_score_canvas)
                .center_justify()
                .set(self.ids.score_title_text, &mut ui_cell);
        }
